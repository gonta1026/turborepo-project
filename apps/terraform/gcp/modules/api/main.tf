...
resource "google_service_account" "api_cloud_run_sa" {
  account_id   = "${var.environment}-api-cloud-run-sa"
  display_name = "API Cloud Run Service Account (${var.environment})"
  description  = "Service account for API Cloud Run service in ${var.environment} environment"
}
...
resource "google_cloud_run_v2_service" "api_service" {
  ...
  template {
    ...
    containers {
      ...
      env {
        name  = "DB_NAME"
        value = var.database_connection_info.database
      }
      ...
      env {
        ...
        value_source {
          secret_key_ref {
            secret  = var.database_connection_info.password_secret
            version = "latest"
          }
        }
      }
...
resource "google_compute_backend_service" "api_backend_service" {
  name                  = "${var.environment}-api-backend-service"
  protocol              = "HTTP"
...
resource "google_certificate_manager_certificate_map_entry" "api_cert_entry" {
  ...
  name         = "${var.environment}-api-cert-entry"
...
resource "google_compute_url_map" "api_http_redirect" {
  ...
  name = "${var.environment}-api-http-redirect"
